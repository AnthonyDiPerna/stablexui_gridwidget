...
Main.main = function() {
	...
	openfl.Lib.current.stage.removeEventListener(openfl.events.Event.ENTER_FRAME,ru.stablex.ui.UIBuilder.skinQueue);
	openfl.Lib.current.stage.addEventListener(openfl.events.Event.ENTER_FRAME,ru.stablex.ui.UIBuilder.skinQueue);
	var totalGridWidth = 400;
	var totalGridHeight = 200;
	var itemWidth = 100;
	var itemHeight = 100;
	var grid = new com.xenizogames.widgets.GridWidget(totalGridWidth,totalGridHeight,itemWidth,itemHeight);
	var _g = 0;
	while(_g < 120) {
		var i = _g++;
		var randomSkin = new ru.stablex.ui.skins.Paint();
		randomSkin.color = Std.random(16777215);
		var button = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button,{ text : "Hello", w : itemWidth, h : itemHeight, skin : randomSkin});
		grid.addWidget(button);
	}
	grid.refresh();
	openfl.Lib.current.addChild(grid);
};
...
ru.stablex.ui.widgets.Widget.prototype = $extend(ru.stablex.TweenSprite.prototype,{
	...
	,set_id: function(id) {
		if(id == null) {
			haxe.Log.trace("Widget id cannot be null",{ fileName : "Err.hx", lineNumber : 18, className : "ru.stablex.Err", methodName : "trigger"});
...
ru.stablex.ui.widgets.Scroll = function() {
	...
	this.addUniqueListener(openfl.events.MouseEvent.MOUSE_WHEEL,$bind(this,this._beforeScroll));
	this.addUniqueListener(openfl.events.MouseEvent.MOUSE_DOWN,$bind(this,this._beforeScroll));
};
...
ru.stablex.ui.widgets.Scroll.prototype = $extend(ru.stablex.ui.widgets.Widget.prototype,{
	get_box: function() {
		if(this.get_numChildren() == 0) {
			haxe.Log.trace("Scroll widget must have at least one child.",{ fileName : "Err.hx", lineNumber : 18, className : "ru.stablex.Err", methodName : "trigger"});
			throw "Scroll widget must have at least one child.";
			return null;
		} else {
			var child = this.getChildAt(0);
			if(!js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)) {
				haxe.Log.trace("Instance of Widget must be the first child for Scroll widget",{ fileName : "Err.hx", lineNumber : 18, className : "ru.stablex.Err", methodName : "trigger"});
	...
	,refresh: function() {
		...
		this.get_box().addUniqueListener("widgetInitialResize",$bind(this,this._update));
		this.get_box().addUniqueListener("widgetResize",$bind(this,this._update));
		this._update();
	}
	,_update: function(e) {
		if(this.vBar != null) {
			if(this.vBar.parent == null) this.addChildAt(this.vBar,1);
			if(this.get_h() - this.get_box().get_h() < 0) this.vBar.min = this.get_h() - this.get_box().get_h(); else this.vBar.min = 0;
			this.vBar.max = 0;
			...
			this.vBar.set_value(this.get_scrollY());
		}
		if(this.hBar != null) {
			...
			this.hBar.min = 0;
			var k1 = this.hBar.get_w() / this.get_box().get_w();
			if(k1 > 1) k1 = 1;
			this.hBar.slider.set_w(this.hBar.get_w() * k1);
			this.hBar.refresh();
			this.hBar.addUniqueListener("widgetChange",$bind(this,this._onHBarChange));
			this.hBar.set_value(-this.get_scrollX());
		}
		if(this.get_scrollY() < this.get_h() - this.get_box().get_h()) this.set_scrollY(this.get_h() - this.get_box().get_h());
		if(this.get_scrollX() < this.get_w() - this.get_box().get_w()) this.set_scrollX(this.get_w() - this.get_box().get_w());
	}
	...
	,_startScroll: function(e) {
		...
		if(e.srcEvent.type == openfl.events.MouseEvent.MOUSE_DOWN && this.dragScroll) this._dragScroll(js.Boot.__instanceof(e.srcEvent,openfl.events.MouseEvent)?e.srcEvent:null); else if(e.srcEvent.type == openfl.events.MouseEvent.MOUSE_WHEEL && this.wheelScroll) this._wheelScroll(js.Boot.__instanceof(e.srcEvent,openfl.events.MouseEvent)?e.srcEvent:null);
	}
	,_dragScroll: function(e) {
		...
		var blocker = new openfl.display.Sprite();
		...
		this.addUniqueListener(openfl.events.Event.ENTER_FRAME,fn);
		var fnStop = null;
		fnStop = function(e2) {
			_g._processingDrag = false;
			_g.removeEventListener(openfl.events.Event.ENTER_FRAME,fn);
			openfl.Lib.current.stage.removeEventListener(openfl.events.MouseEvent.MOUSE_UP,fnStop);
			if(scrolled) {
				var finish = function() {
		...
		openfl.Lib.current.stage.removeEventListener(openfl.events.MouseEvent.MOUSE_UP,fnStop);
		openfl.Lib.current.stage.addEventListener(openfl.events.MouseEvent.MOUSE_UP,fnStop);
	}
	,_wheelScroll: function(e) {
		if(this.hScroll && (e.altKey && this.hScrollKey == "alt" || e.shiftKey && this.hScrollKey == "shift" || e.ctrlKey && this.hScrollKey == "ctrl")) {
			...
			this.tweenStop();
			var _g1 = this;
			_g1.set_scrollY(_g1.get_scrollY() + e.delta * this.wheelScrollSpeed);
		}
	}
	,__class__: ru.stablex.ui.widgets.Scroll
	,__properties__: $extend(ru.stablex.ui.widgets.Widget.prototype.__properties__,{set_hBar:"set_hBar",set_vBar:"set_vBar",set_scrollY:"set_scrollY",get_scrollY:"get_scrollY",set_scrollX:"set_scrollX",get_scrollX:"get_scrollX",get_box:"get_box"})
});
var com = {};
com.xenizogames = {};
com.xenizogames.widgets = {};
com.xenizogames.widgets.GridWidget = function(inWidth,inHeight,inItemWidth,inItemHeight) {
	ru.stablex.ui.widgets.Scroll.call(this);
	this._itemHeight = inItemHeight;
	this._itemWidth = inItemHeight;
	this.addEventListener("widgetScrollStart",$bind(this,this.onScrollStart));
	this.addEventListener("widgetScrollStop",$bind(this,this.onScrollStop));
	this._numCols = Math.floor(inWidth / inItemWidth);
	this._currIndex = 0;
	this._numRows = 0;
	this.set_w(inWidth);
	this.set_h(inHeight);
	this._scrollHeight = inHeight;
	this.gridList = new ru.stablex.ui.widgets.HBox();
	this.gridList.set_widthPt(100);
	this.gridList.set_heightPt(100);
	this.addChild(this.gridList);
	this.gridSetup();
	this.refresh();
	motion.Actuate.tween(this.vBar,2.0,{ alpha : 0});
};
$hxClasses["com.xenizogames.widgets.GridWidget"] = com.xenizogames.widgets.GridWidget;
com.xenizogames.widgets.GridWidget.__name__ = ["com","xenizogames","widgets","GridWidget"];
com.xenizogames.widgets.GridWidget.__super__ = ru.stablex.ui.widgets.Scroll;
com.xenizogames.widgets.GridWidget.prototype = $extend(ru.stablex.ui.widgets.Scroll.prototype,{
	onScrollStart: function(e) {
		motion.Actuate.stop(this.vBar);
		motion.Actuate.tween(this.vBar,1.0,{ alpha : 1});
	}
	,onScrollStop: function(e) {
		motion.Actuate.stop(this.vBar);
		motion.Actuate.tween(this.vBar,1.0,{ alpha : 0});
	}
	,gridSetup: function() {
		var columnLayout = new ru.stablex.ui.layouts.Column();
		...
		var _g1 = 0;
		var _g = this._numCols;
		while(_g1 < _g) {
			var n = _g1++;
			var b = new ru.stablex.ui.widgets.VBox();
			b.childPadding = 5;
			b.set_name("col" + Std.string(n + 1));
			b.align = "center,top";
			this.gridList.addChild(b);
			this.columns.push(b);
		}
	}
	,addToEndOfGrid: function(inWidget) {
		this.addWidget(inWidget);
		this.resortGrid();
	}
	,addtoStartOfGrid: function(inWidget) {
		this.columns[0].addChildAt(inWidget,1);
		this.resortGrid();
	}
	,addtoMiddleOfGrid: function(inWidget) {
		var firstCol = this.columns[0];
		var insertPos;
		if(firstCol.get_numChildren() > 0) insertPos = Math.floor(firstCol.get_numChildren() / 2); else insertPos = 0;
	...
	,checkForUnfinishedRows: function() {
		if(this._currIndex > 0) this._numRows++;
	}
	,resortGrid: function() {
		var items = new Array();
		var totalChildren = 0;
		...
		while(_g < _g1.length) {
			var c = _g1[_g];
			++_g;
			if(totalChildren < c.get_numChildren()) totalChildren = c.get_numChildren();
		}
		var _g2 = 0;
		while(_g2 < totalChildren) {
			var z = _g2++;
			var _g11 = 0;
			...
			while(_g11 < _g21.length) {
				var col = _g21[_g11];
				++_g11;
				if(col.get_numChildren() > 0) {
					var item = col.getChildAt(0);
					if(js.Boot.__instanceof(item,ru.stablex.ui.widgets.Widget)) {
						items.push(item);
		...
		while(_g3 < items.length) {
			var i = items[_g3];
			++_g3;
			this.columns[this._currIndex].addChild(i);
			this._currIndex++;
			if(this._currIndex >= this._numCols) {
				this._currIndex = 0;
				this._numRows++;
	...
	,getScrollHeight: function(inNumRows,inHeight) {
		return Math.floor(Math.max(inNumRows * inHeight,this._scrollHeight));
	}
	,refresh: function() {
		this.checkForUnfinishedRows();
		this.gridList.set_h(Math.floor(Math.max((this._numRows + 1) * this._itemHeight,this._scrollHeight)));
		var _g = 0;
		var _g1 = this.columns;
		while(_g < _g1.length) {
			var c = _g1[_g];
		...
		ru.stablex.ui.widgets.Scroll.prototype.refresh.call(this);
	}
	,getColumnArrayForGrid: function(inNumCols) {
		var cols = new Array();
		...
		if(cols.length < 2) throw "error...not enough space to make columns correctly...layout is borked!";
		return cols;
	}
	,__class__: com.xenizogames.widgets.GridWidget
});
var haxe = {};
...
ru.stablex.Err.trigger = function(msg) {
	haxe.Log.trace(msg,{ fileName : "Err.hx", lineNumber : 18, className : "ru.stablex.Err", methodName : "trigger"});
	throw msg;
};
...
ru.stablex.ui.events.ScrollEvent.__super__ = openfl.events.Event;
ru.stablex.ui.events.ScrollEvent.prototype = $extend(openfl.events.Event.prototype,{
	cancel: function() {
		this.canceled = true;
	}
	,srcAs: function(cls) {
		if(js.Boot.__instanceof(this.srcEvent,cls)) return this.srcEvent; else return null;
	}
...
ru.stablex.ui.layouts.Column = function() {
	this.fit = true;
	this.cellPadding = 0;
	...
	this.paddingRight = 0;
	this.paddingLeft = 0;
	ru.stablex.ui.layouts.Layout.call(this);
};
$hxClasses["ru.stablex.ui.layouts.Column"] = ru.stablex.ui.layouts.Column;
ru.stablex.ui.layouts.Column.__name__ = ["ru","stablex","ui","layouts","Column"];
ru.stablex.ui.layouts.Column.__super__ = ru.stablex.ui.layouts.Layout;
ru.stablex.ui.layouts.Column.prototype = $extend(ru.stablex.ui.layouts.Layout.prototype,{
	arrangeChildren: function(holder) {
		if(this.cols == null || this.cols.length == 0) return;
		var acols = [];
		var width = holder.get_w() - this.paddingLeft - this.paddingRight - (this.cols.length - 1) * this.cellPadding;
		var negParts = 0;
		var freeSpace = width;
		var _g1 = 0;
		...
		while(_g1 < _g) {
			var i = _g1++;
			if(this.cols[i] > 1) freeSpace -= acols[i] = this.cols[i]; else if(this.cols[i] < 0) negParts += acols[i] = this.cols[i]; else freeSpace -= acols[i] = width * this.cols[i];
		}
		var _g11 = 0;
		var _g2 = acols.length;
		while(_g11 < _g2) {
			var i1 = _g11++;
			if(acols[i1] < 0) acols[i1] = freeSpace * acols[i1] / negParts;
		}
		var child;
		var left = this.paddingLeft;
		var _g12 = 0;
		var _g3 = acols.length;
		while(_g12 < _g3) {
			var i2 = _g12++;
			if(holder.get_numChildren() <= i2) break;
			child = holder.getChildAt(i2);
			if(js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)) {
				(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).set_left(left);
				(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).set_top(this.paddingTop);
			} else {
				child.set_x(left);
				child.set_y(this.paddingTop);
			}
			if(this.fit && js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)) (js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).resize(acols[i2],holder.get_h() - this.paddingTop - this.paddingBottom);
			left += acols[i2] + this.cellPadding;
		}
	}
	,set_padding: function(padding) {
		return this.paddingLeft = this.paddingRight = this.paddingTop = this.paddingBottom = padding;
	}
	,__class__: ru.stablex.ui.layouts.Column
...
ru.stablex.ui.widgets.Bmp.prototype = $extend(ru.stablex.ui.widgets.Widget.prototype,{
	refresh: function() {
		...
		if(bmp != null) {
			if(this.autoWidth && this.autoHeight && (this._width != (this.drawPortion?bmp.width - this.xOffset:bmp.width) || this._height != (this.drawPortion?bmp.height - this.yOffset:bmp.height))) this.resize(bmp.width,bmp.height); else if(this.autoWidth && this._width != (this.drawPortion?bmp.width - this.xOffset:bmp.width)) this.set_w(this.drawPortion?bmp.width - this.xOffset:bmp.width); else if(this.autoHeight && this._height != (this.drawPortion?bmp.height - this.yOffset:bmp.height)) this.set_h(this.drawPortion?bmp.height - this.yOffset:bmp.height);
			ru.stablex.ui.widgets.Widget.prototype.refresh.call(this);
			this._draw(bmp);
	...
	,_draw: function(bmp) {
		...
		if(this.drawPortion) {
			var width;
			if(this.autoWidth) width = bmp.width; else if(this._width > bmp.width - this.xOffset) width = bmp.width - this.xOffset; else width = this._width;
			var height;
			if(this.autoHeight) height = bmp.height; else if(this._height > bmp.height - this.yOffset) height = bmp.height - this.yOffset; else height = this._height | 0;
			if(width <= 0 || height <= 0) return; else {
				var mx = new openfl.geom.Matrix();
				var dest = new openfl.display.BitmapData(width | 0,height | 0);
				dest.copyPixels(bmp,new openfl.geom.Rectangle(this.xOffset,this.yOffset,width,height),new openfl.geom.Point(0,0));
				bmp = dest;
			...
			this.get_graphics().drawRect(0,0,bmp.width,bmp.height);
			this.get_graphics().endFill();
		}
	}
	,get_src: function() {
	...
	,set_xOffset: function(x) {
		...
		return x >= 0?this.xOffset = x:this.xOffset = 0;
	}
	,set_yOffset: function(y) {
		this.drawPortion = true;
		return y >= 0?this.yOffset = y:this.yOffset = 0;
	}
	,set_w: function(w) {
	...
	,__properties__: $extend(ru.stablex.ui.widgets.Widget.prototype.__properties__,{set_yOffset:"set_yOffset",set_xOffset:"set_xOffset",set_bitmapData:"set_bitmapData",get_bitmapData:"get_bitmapData",set_autoSize:"set_autoSize",set_src:"set_src",get_src:"get_src"})
});
...
ru.stablex.ui.widgets.Box._objHeight = function(obj) {
	if(js.Boot.__instanceof(obj,ru.stablex.ui.widgets.Widget)) return (js.Boot.__cast(obj , ru.stablex.ui.widgets.Widget)).get_h(); else if(js.Boot.__instanceof(obj,openfl.text.TextField)) return (js.Boot.__cast(obj , openfl.text.TextField)).get_textHeight() + 4; else return obj.get_height();
};
		...
		if(this.vertical) {
			...
			while(_g1 < _g) {
				...
				if(child.get_visible()) {
					if(js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)) childW = (js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_w(); else if(js.Boot.__instanceof(child,openfl.text.TextField)) childW = (js.Boot.__cast(child , openfl.text.TextField)).get_textWidth() + 4; else childW = child.get_width();
			...
			while(_g11 < _g2) {
				...
				if(child1.get_visible()) {
					if(js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)) w1 += (js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_w(); else if(js.Boot.__instanceof(child1,openfl.text.TextField)) w1 += (js.Boot.__cast(child1 , openfl.text.TextField)).get_textWidth() + 4; else w1 += child1.get_width();
					visibleChildren++;
				}
			}
	...
	,_calcHeight: function() {
		if(this.vertical) {
			...
			while(_g1 < _g) {
				...
				child = this.getChildAt(i);
				if(child.get_visible()) {
					if(js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)) h += (js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_h(); else if(js.Boot.__instanceof(child,openfl.text.TextField)) h += (js.Boot.__cast(child , openfl.text.TextField)).get_textHeight() + 4; else h += child.get_height();
					visibleChildren++;
				}
			}
			...
			while(_g11 < _g2) {
				...
				child1 = this.getChildAt(i1);
				if(child1.get_visible()) {
					if(js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)) childH = (js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_h(); else if(js.Boot.__instanceof(child1,openfl.text.TextField)) childH = (js.Boot.__cast(child1 , openfl.text.TextField)).get_textHeight() + 4; else childH = child1.get_height();
	...
	,_vAlignTop: function() {
		if(this.vertical) {
			...
			while(_g1 < _g) {
				...
				child.set_y(js.Boot.__instanceof(child,openfl.text.TextField)?child.set_y(lastY + 2):lastY);
				lastY += (js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_h():js.Boot.__instanceof(child,openfl.text.TextField)?(js.Boot.__cast(child , openfl.text.TextField)).get_textHeight() + 4:child.get_height()) + this.childPadding;
			}
		} else {
			var _g11 = 0;
			var _g2 = this.get_numChildren();
			while(_g11 < _g2) {
				var i1 = _g11++;
				ru.stablex.ui.widgets.Box._setObjY(this.getChildAt(i1),this.paddingTop);
			}
		}
	}
	,_vAlignMiddle: function() {
		if(this.vertical) {
			...
			while(_g1 < _g) {
				...
				if(!child.get_visible()) continue;
				visibleChildren++;
				if(js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)) height += (js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_h(); else if(js.Boot.__instanceof(child,openfl.text.TextField)) height += (js.Boot.__cast(child , openfl.text.TextField)).get_textHeight() + 4; else height += child.get_height();
			}
			...
			while(_g12 < _g3) {
				...
				ru.stablex.ui.widgets.Box._setObjY(child1,(this.get_h() - (js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_h():js.Boot.__instanceof(child1,openfl.text.TextField)?(js.Boot.__cast(child1 , openfl.text.TextField)).get_textHeight() + 4:child1.get_height())) / 2);
			}
		}
	}
	,_vAlignBottom: function() {
		if(this.vertical) {
			...
			while(_g1 < _g) {
				...
				if(!child.get_visible()) continue;
				ru.stablex.ui.widgets.Box._setObjY(child,lastY - (js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_h():js.Boot.__instanceof(child,openfl.text.TextField)?(js.Boot.__cast(child , openfl.text.TextField)).get_textHeight() + 4:child.get_height()));
			...
			while(_g11 < _g2) {
				var i1 = _g11++;
				child1 = this.getChildAt(i1);
				ru.stablex.ui.widgets.Box._setObjY(child1,this.get_h() - this.paddingBottom - (js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_h():js.Boot.__instanceof(child1,openfl.text.TextField)?(js.Boot.__cast(child1 , openfl.text.TextField)).get_textHeight() + 4:child1.get_height()));
			}
		}
	}
	,_hAlignLeft: function() {
		if(this.vertical) {
			...
			while(_g11 < _g2) {
				...
				child = this.getChildAt(i1);
				if(!child.get_visible()) continue;
				child.set_x(js.Boot.__instanceof(child,openfl.text.TextField)?child.set_x(lastX + 2):lastX);
				lastX += (js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_w():js.Boot.__instanceof(child,openfl.text.TextField)?(js.Boot.__cast(child , openfl.text.TextField)).get_textWidth() + 4:child.get_width()) + this.childPadding;
			}
		}
	}
	,_hAlignRight: function() {
		if(this.vertical) {
			...
			while(_g1 < _g) {
				var i = _g1++;
				child = this.getChildAt(i);
				ru.stablex.ui.widgets.Box._setObjX(child,this.get_w() - this.paddingRight - (js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_w():js.Boot.__instanceof(child,openfl.text.TextField)?(js.Boot.__cast(child , openfl.text.TextField)).get_textWidth() + 4:child.get_width()));
			}
		} else {
			...
			while(_g11 < _g2) {
				...
				ru.stablex.ui.widgets.Box._setObjX(child1,lastX - (js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_w():js.Boot.__instanceof(child1,openfl.text.TextField)?(js.Boot.__cast(child1 , openfl.text.TextField)).get_textWidth() + 4:child1.get_width()));
	...
	,_hAlignCenter: function() {
		if(this.vertical) {
			...
			while(_g1 < _g) {
				var i = _g1++;
				child = this.getChildAt(i);
				ru.stablex.ui.widgets.Box._setObjX(child,(this.get_w() - (js.Boot.__instanceof(child,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child , ru.stablex.ui.widgets.Widget)).get_w():js.Boot.__instanceof(child,openfl.text.TextField)?(js.Boot.__cast(child , openfl.text.TextField)).get_textWidth() + 4:child.get_width())) / 2);
			}
		} else {
			var child1;
			var width = 0;
			...
			while(_g11 < _g2) {
				...
				if(!child1.get_visible()) continue;
				visibleChildren++;
				if(js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)) width += (js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_w(); else if(js.Boot.__instanceof(child1,openfl.text.TextField)) width += (js.Boot.__cast(child1 , openfl.text.TextField)).get_textWidth() + 4; else width += child1.get_width();
			}
			...
			while(_g12 < _g3) {
				...
				child1 = this.getChildAt(i2);
				if(!child1.get_visible()) continue;
				child1.set_x(js.Boot.__instanceof(child1,openfl.text.TextField)?child1.set_x(lastX + 2):lastX);
				lastX += (js.Boot.__instanceof(child1,ru.stablex.ui.widgets.Widget)?(js.Boot.__cast(child1 , ru.stablex.ui.widgets.Widget)).get_w():js.Boot.__instanceof(child1,openfl.text.TextField)?(js.Boot.__cast(child1 , openfl.text.TextField)).get_textWidth() + 4:child1.get_width()) + this.childPadding;
			}
		}
	}
	,addChild: function(child) {
...
ru.stablex.ui.widgets.Button = function() {
	...
	this.addEventListener(openfl.events.MouseEvent.MOUSE_OVER,ru.stablex.ui.widgets.Button._onHover);
	this.addEventListener(openfl.events.MouseEvent.MOUSE_OUT,ru.stablex.ui.widgets.Button._onHout);
	this.addEventListener(openfl.events.MouseEvent.MOUSE_DOWN,ru.stablex.ui.widgets.Button._onPress);
	this.addEventListener(openfl.events.MouseEvent.MOUSE_OUT,ru.stablex.ui.widgets.Button._onRelease);
	this.addEventListener(openfl.events.MouseEvent.MOUSE_UP,ru.stablex.ui.widgets.Button._onRelease);
	this.pressed = false;
	this.hovered = false;
	this.align = "center,middle";
};
...
ru.stablex.ui.widgets.Button._onHover = function(e) {
	var btn;
	btn = js.Boot.__cast(e.currentTarget , ru.stablex.ui.widgets.Button);
...
ru.stablex.ui.widgets.Button.prototype = $extend(ru.stablex.ui.widgets.Text.prototype,{
	...
	,set_icoBeforeLabel: function(ibl) {
		if(ibl) this.setChildIndex(this.label,this.get_numChildren() - 1); else this.setChildIndex(this.label,0);
	...
	,__properties__: $extend(ru.stablex.ui.widgets.Text.prototype.__properties__,{set_skinPressedName:"set_skinPressedName",set_skinHoveredName:"set_skinHoveredName",set_icoBeforeLabel:"set_icoBeforeLabel",set_icoPressed:"set_icoPressed",get_icoPressed:"get_icoPressed",set_icoHovered:"set_icoHovered",get_icoHovered:"get_icoHovered",set_ico:"set_ico",get_ico:"get_ico"})
});
...
ru.stablex.ui.widgets.Floating.prototype = $extend(ru.stablex.ui.widgets.Box.prototype,{
	show: function() {
		if(this.renderTo == null) {
			...
			if(this.parent == openfl.Lib.current) openfl.Lib.current.stage.setChildIndex(this,openfl.Lib.current.stage.get_numChildren() - 1); else openfl.Lib.current.stage.addChild(this);
			this._onStageResize();
		} else {
			var to = ru.stablex.ui.UIBuilder._objects.get(this.renderTo);
			if(to != null) {
				this.set_visible(this.shown = true);
				if(this.parent == to) to.setChildIndex(this,to.get_numChildren() - 1); else to.addChild(this);
			}
		}
	}
	,hide: function() {
...
ru.stablex.ui.widgets.Slider = function() {
	...
	this.slider.addEventListener(openfl.events.MouseEvent.MOUSE_DOWN,$bind(this,this._slide));
	this.addEventListener(openfl.events.MouseEvent.CLICK,$bind(this,this._set));
};
...
ru.stablex.ui.widgets.Slider.prototype = $extend(ru.stablex.ui.widgets.Widget.prototype,{
	get_value: function() {
		if(this._value < this.min) return this.min; else if(this._value > this.max) return this.max; else return this._value;
	}
	...
	,_set: function(e) {
		if(e.target != this.slider) {
			if(this.vertical) {
				var y;
				if(this.get_mouseY() - this.slider._height / 2 < 0) y = 0; else if(this.get_mouseY() + this.slider._height / 2 > this._height) y = this._height - this.slider._height; else y = this.get_mouseY() - this.slider._height / 2;
				this.slider.tween(0.25,{ top : y},"Quad.easeOut");
				this._value = (1 - y / (this._height - this.slider._height)) * (this.max - this.min) + this.min;
			} else {
				var x;
				if(this.get_mouseX() - this.slider._width / 2 < 0) x = 0; else if(this.get_mouseX() + this.slider._width / 2 > this._width) x = this._width - this.slider._width; else x = this.get_mouseX() - this.slider._width / 2;
	...
	,_slideVertically: function() {
		...
		var fn = function(e) {
			var y;
			if(_g.get_mouseY() - dy < 0) y = 0; else if(_g.get_mouseY() - dy + _g.slider._height > _g._height) y = _g._height - _g.slider._height; else y = _g.get_mouseY() - dy;
		...
		this.addEventListener(openfl.events.Event.ENTER_FRAME,fn);
		var fnRelease = null;
		fnRelease = function(e1) {
			_g.removeEventListener(openfl.events.Event.ENTER_FRAME,fn);
			openfl.Lib.current.stage.removeEventListener(openfl.events.MouseEvent.MOUSE_UP,fnRelease);
		};
		openfl.Lib.current.stage.addEventListener(openfl.events.MouseEvent.MOUSE_UP,fnRelease);
	}
	,_slideHorizontally: function() {
		...
		var fn = function(e) {
			var x;
			if(_g.get_mouseX() - dx < 0) x = 0; else if(_g.get_mouseX() - dx + _g.slider._width > _g._width) x = _g._width - _g.slider._width; else x = _g.get_mouseX() - dx;
